...
Computed with taste shocks (discrete choice methods) for $B'$ and $d$. Fortran parallelized with OpenMP. A Python version (with NumPy, Numba, and Matplotlib) as well as a Python/JAX version are available as well, ported by Grok 4.5 with minimual human supervision. See PYTHON.md for more info.
